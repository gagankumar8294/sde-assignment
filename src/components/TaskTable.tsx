...
export default function TaskTable({ tasks, onAdd, onUpdate, onDelete }: Props) {
  ...
  const handleEditClick = (task: Task, e?: React.MouseEvent) => {
    if (e) e.stopPropagation(); // avoiding opening details popup
    setEditing(task);
    setOpenForm(true);
  };
                  ...
                  <TableCell align="right">
                    <Stack direction="row" spacing={1} justifyContent="flex-end">
                      <Tooltip title="Edit">
                        <IconButton
                          size="small"
                          onClick={(e) => handleEditClick(t, e)} // 👈 stop event propagation
                        >
                      ...
                      <Tooltip title="Delete">
                        <IconButton
                          size="small"
                          color="error"
                          onClick={(e) => handleDeleteClick(t.id, e)} // 👈 stop event propagation
                        >
